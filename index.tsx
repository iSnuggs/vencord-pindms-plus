...
import { canMoveCategory, canMoveCategoryInDirection, Category, categoryLen, collapseCategory, getAllUncollapsedChannels, getCategoryByIndex, getSections, init, isGroupDmsCategory, isPinned, moveCategory, removeCategory, usePinnedDms } from "./data";
...
export const settings = definePluginSettings({
    ...
    groupDmsAutoCategory: {
        type: OptionType.BOOLEAN,
        displayName: "Group Chats Category",
        description: "Automatically collect every group DM you're in into its own collapsible \"Group Chats\" section",
        default: true
    },
    groupDmsCategoryCollapsed: {
        type: OptionType.BOOLEAN,
        displayName: "Group Chats Category Collapsed",
        description: "Collapse the Group Chats section",
        default: false,
        hidden: true
    }
});
    ...
    renderCategory: ErrorBoundary.wrap(({ section }: { section: number; }) => {
        ...
        return (
            <Clickable
                ...
                onContextMenu={e => {
                    // The auto-populated Group Chats category isn't user-editable -
                    // membership is derived live from your group DMs, not curated,
                    // so Edit/Move/Delete don't apply to it.
                    if (isGroupDmsCategory(category.id)) return;
